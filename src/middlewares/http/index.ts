...
const secretadmin=process.env.SECRETADMIN||"d"
/**
 * 
 ...
 */
export const verifyTokenclient=(req:Request,res:Response,next:NextFunction)=>{
   //   check
...
export const verifytokenadminandclient=(req:Request,res:Response,next:NextFunction)=>{
  const token=req.header("x-access-token")||"s";
   console.log(token)
   ...
   jwt.verify(token,secret,(err:any,decoded:any)=>{
    if(err){
      console.log("error")
      jwt.verify(token,secretadmin,(err:any,decoded:any)=>{
        console.log("error")
        if(err){
          return res.status(401).json({message:"no autorizado"})
        }
